...
void draw(){
  ...
  for(int x=0;x<width;x++){ //forloop for X value
     for(int y=0;y<height;y++){
       ...
       float d=dist(mouseX,mouseY,x,y); // follows mouse movment
       float f=map(d,0,200,2,0);  // Do not completly understand map and what value it will output.
       
       pixels[loc] = color(r*f,g*f,b*f);// multiplies tiwh F (factor)
     }
  }
  updatePixels();
}